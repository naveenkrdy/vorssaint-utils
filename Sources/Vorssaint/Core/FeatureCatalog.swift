...
enum AppFeature: String, CaseIterable {
    ...
    case quickLauncher, quickToggles, colorPicker, screenOCR, cleaningMode, mediaTools,
         ...
         commandBar, screenRecorder, killProcess, quarantineManager
...
extension AppFeature {
    ...
    var group: FeatureGroup {
        ...
        case .quickLauncher, .quickToggles, .colorPicker, .screenOCR, .cleaningMode, .mediaTools,
             ...
             .scratchpad, .commandBar, .screenRecorder, .killProcess, .quarantineManager:
            return .tools
    ...
    var isBeta: Bool { self == .fanControl || self == .killProcess || self == .quarantineManager }
        ...
        Dictionary(uniqueKeysWithValues: allCases.map {
            ($0.availabilityKey,
             $0 != .focusFollowsMouse && $0 != .fanControl && $0 != .diskImageInstaller
                && $0 != .killProcess && $0 != .quarantineManager)
        })
    }
